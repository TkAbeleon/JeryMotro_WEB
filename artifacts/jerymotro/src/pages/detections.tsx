...
export default function DetectionsPage() {
  ...
  return (
    <div className="min-h-full bg-background px-4 py-5 sm:px-6 sm:py-7 lg:px-8"><div className="mx-auto flex h-full max-w-[1600px] flex-col gap-6">
      ...
      <section className="rounded-xl border border-border/70 bg-card/55 p-3 shadow-sm sm:p-4"><div className="flex flex-wrap items-center gap-2.5">
        <div className="relative min-w-[220px] flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("detections.search.placeholder")} className="h-9 w-full rounded-lg border border-input bg-background pl-9 pr-3 text-sm outline-none transition-all placeholder:text-muted-foreground/60 focus:border-primary/40 focus:ring-2 focus:ring-primary/10" /></div>
        <label className="flex h-9 items-center gap-2 rounded-lg border border-input bg-background px-2.5 text-xs text-muted-foreground"><Filter className="h-3.5 w-3.5" /><select value={region} onChange={e => setRegion(e.target.value)} className="bg-transparent text-sm text-foreground outline-none">{REGIONS.map(r => <option key={r} value={r}>{r === "Toutes" ? allLabel : r}</option>)}</select></label>
        <select value={source} onChange={e => setSource(e.target.value)} className="h-9 rounded-lg border border-input bg-background px-3 text-sm outline-none transition-colors focus:border-primary/40 focus:ring-2 focus:ring-primary/10">{SOURCES.map(s => <option key={s} value={s}>{s === "Toutes" ? t("detections.filter.allSources") : s}</option>)}</select>
      ...
      <div className="relative flex min-h-0 flex-1 gap-4">
        ...
        {selectedDet && <aside className="absolute inset-y-0 right-0 z-10 w-full overflow-y-auto rounded-xl border border-border bg-card/95 p-5 shadow-2xl backdrop-blur-xl sm:static sm:w-80 sm:shrink-0 sm:shadow-sm">
