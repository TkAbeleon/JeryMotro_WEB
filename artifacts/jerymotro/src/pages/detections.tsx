...
export default function DetectionsPage() {
  ...
  if (query.isLoading) return <AsyncStateInline type="loading" title={t("common.loading")} description="Chargement des détections…" />;
  if (query.isError) return <AsyncStateInline type="error" title="Impossible de charger les détections" description="Vérifiez votre connexion puis réessayez." onAction={() => query.refetch()} actionLabel="Réessayer" />;
  ...
  return (
    <div className="min-h-full bg-background px-4 py-5 sm:px-6 sm:py-7 lg:px-8"><div className="mx-auto flex h-full max-w-[1600px] flex-col gap-6">
      ...
      <section className="jm-filter-shell rounded-2xl border border-border/70 bg-card/55 p-3 shadow-sm sm:p-4"><div className="flex flex-wrap items-center gap-2.5">
        <div className="relative min-w-[220px] flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("detections.search.placeholder")} className="jm-control-inset h-9 w-full rounded-2xl border border-input bg-background pl-9 pr-3 text-sm outline-none placeholder:text-muted-foreground/60 focus:ring-0" /></div>
        <label className="jm-control-inset flex h-9 items-center gap-2 rounded-2xl border border-input bg-background px-2.5 text-xs text-muted-foreground"><Filter className="h-3.5 w-3.5" /><select value={region} onChange={e => setRegion(e.target.value)} className="bg-transparent text-sm text-foreground outline-none">{REGIONS.map(r => <option key={r} value={r}>{r === "Toutes" ? allLabel : r}</option>)}</select></label>
        <select value={source} onChange={e => setSource(e.target.value)} className="jm-control-inset h-9 rounded-2xl border border-input bg-background px-3 text-sm outline-none focus:ring-0">{SOURCES.map(s => <option key={s} value={s}>{s === "Toutes" ? t("detections.filter.allSources") : s}</option>)}</select>
        {(region !== "Toutes" || source !== "Toutes" || search) && <button onClick={() => { setRegion("Toutes"); setSource("Toutes"); setSearch(""); }} className="jm-button-depth inline-flex h-9 items-center gap-1.5 rounded-xl px-2.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><X className="h-3 w-3" />{t("common.reset")}</button>}
      </div>

      <div className="relative flex min-h-0 flex-1 gap-4">
        <div className={`jm-card-depth ${selected ? "hidden sm:flex sm:flex-1" : "flex w-full"} min-h-0 flex-col overflow-hidden rounded-xl border border-border/70 bg-card/55 shadow-sm`}>
          <div className="min-h-0 flex-1 overflow-x-auto">
            <div className="hidden min-w-[650px] grid-cols-[56px_minmax(0,1fr)_minmax(0,1fr)_80px_64px_80px] gap-4 border-b border-border/60 bg-muted/20 px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground sm:grid"><span>{t("detections.col.id")}</span><span>{t("detections.col.region")}</span><span>{t("detections.col.coords")}</span><span className="text-right">{t("detections.col.frp")}</span><span className="text-right">{t("detections.col.source")}</span><span className="text-right">{t("detections.col.risk")}</span></div>
            <div className="min-h-full divide-y divide-border/45 sm:min-w-[650px]">
              {filtered.length === 0 ? <AsyncStateInline type="empty" title={t("detections.empty")} description="Essayez de modifier ou réinitialiser les filtres." className="sm:min-w-0" /> : filtered.map(d => (
                <button key={d.id} data-testid={`row-detection-${d.id}`} onClick={() => setSelected(d.id === selected ? null : d.id)} className={`jm-detection-row w-full items-center gap-4 px-4 py-3.5 text-left transition-colors sm:grid sm:grid-cols-[56px_minmax(0,1fr)_minmax(0,1fr)_80px_64px_80px] ${selected === d.id ? "jm-detection-row-selected bg-primary/[0.045]" : "hover:bg-muted/35"}`}>
                  <div className="sm:hidden"><div className="mb-1 flex items-center justify-between gap-2"><span className="truncate text-sm font-medium">{d.region || "—"}</span><span className={`jm-pill-depth shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${getRiskColor(d.risk_score)}`}>{getRiskLabel(d.risk_score)}</span></div><div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground"><span>#{d.id}</span><span>•</span><span className="font-mono">{d.latitude.toFixed(3)}, {d.longitude.toFixed(3)}</span><span>•</span><span>{d.source}</span><span>•</span><span>{d.frp?.toFixed(0) ?? "—"} MW</span></div></div>
                  <span className="hidden text-xs text-muted-foreground sm:inline">{d.id}</span><span className="hidden truncate text-sm font-medium sm:inline">{d.region || "—"}</span><span className="hidden font-mono text-xs text-muted-foreground sm:inline">{d.latitude.toFixed(3)}, {d.longitude.toFixed(3)}</span><span className="hidden text-right font-mono text-xs sm:inline">{d.frp?.toFixed(0) ?? "—"}</span><span className="hidden text-right text-xs sm:inline">{d.source}</span><span className="hidden text-right sm:inline"><span className={`jm-pill-depth rounded-full px-2 py-0.5 text-[11px] font-semibold ${getRiskColor(d.risk_score)}`}>{getRiskLabel(d.risk_score)}</span></span>
                </button>
              ))}
            </div>
          </div>
        ...
        {selectedDet && <aside className="jm-detail-panel absolute inset-y-0 right-0 z-10 w-full overflow-y-auto rounded-2xl border border-border bg-card/95 p-5 shadow-2xl backdrop-blur-xl sm:static sm:w-80 sm:shrink-0 sm:shadow-sm">
