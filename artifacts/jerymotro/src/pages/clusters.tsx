...
export default function ClustersPage() {
  ...
  return (
    <div className="min-h-full bg-background px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
      <header className="mb-6 flex flex-col gap-4 border-b border-border/60 pb-5 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-heading text-2xl font-semibold tracking-tight">{t("clusters.title")}</h1><p className="text-sm text-muted-foreground mt-1">{t("clusters.subtitle")}</p></div></header>
      ...
      {filtered.length === 0 ? <AsyncState type="empty" title={search || statusFilter !== "all" ? "Aucun cluster correspondant" : t("clusters.empty")} description={search || statusFilter !== "all" ? "Modifiez la recherche ou le filtre pour voir d’autres résultats." : undefined} /> : <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">{filtered.map(c => {
        ...
        return <div key={c.id} data-testid={`card-cluster-${c.id}`} className={`jm-card-depth rounded-xl border border-border/70 bg-card/65 p-5 shadow-sm transition-all hover:bg-card ${statusLower === "active" ? "border-l-2 border-l-destructive" : ""}`}>
          <div className="flex items-start justify-between mb-3"><div><div className="flex items-center gap-2"><span className="font-heading font-bold text-sm">{c.fire_id || `Cluster #${c.id}`}</span>{(c.reactivation_count ?? 0) > 0 && <span className="jm-pill-depth text-[10px] bg-[#f59e0b]/15 text-[#f59e0b] px-1.5 py-0.5 rounded-full font-medium">{c.reactivation_count}x {t("clusters.card.reactivated")}</span>}</div><div className="flex items-center gap-1 text-xs text-muted-foreground mt-1"><MapPin className="w-3 h-3" /><span>{c.region || t("clusters.card.unknownRegion")}</span></div></div><span className={`jm-pill-depth text-xs font-semibold px-2 py-1 rounded-full ${status.color}`}>{status.label}</span></div>
