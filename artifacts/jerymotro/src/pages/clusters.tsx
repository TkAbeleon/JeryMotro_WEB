        ...
        <div className="flex gap-2">
          {filterButtons.map(f => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${statusFilter === f.key ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}
            >
              {f.label}
            </button>
          ))}
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-heading font-bold text-sm">{c.fire_id || `Cluster #${c.id}`}</span>
                    {(c.reactivation_count ?? 0) > 0 && (
                      <span className="text-[10px] bg-[#f59e0b]/15 text-[#f59e0b] px-1.5 py-0.5 rounded-full font-medium">
                        {c.reactivation_count}x {t("clusters.card.reactivated")}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                    <MapPin className="w-3 h-3" />
                    <span>{c.region || t("clusters.card.unknownRegion")}</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-3">
                <div>
                  <div className="text-xs text-muted-foreground">{t("clusters.card.size")}</div>
                  <div className="font-heading font-bold text-sm">{c.cluster_size ?? "—"} {t("clusters.card.points")}</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">{t("clusters.card.frpMax")}</div>
                  <div className="font-heading font-bold text-sm">{c.cluster_frp_max?.toFixed(0) ?? "—"} MW</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">{t("clusters.card.duration")}</div>
                  <div className="font-heading font-bold text-sm">{c.duration_hours?.toFixed(0) ?? "—"}h</div>
                </div>
              </div>

              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div className={`w-2 h-2 rounded-full ${c.risk_score_max && c.risk_score_max >= 0.7 ? "bg-destructive" : c.risk_score_max && c.risk_score_max >= 0.5 ? "bg-primary" : "bg-[#f59e0b]"}`} />
                  <span className={`text-xs font-bold ${riskColor.color}`}>{c.risk_level || "—"}</span>
                  <span className="text-xs text-muted-foreground">({((c.risk_score_max ?? 0) * 100).toFixed(0)}%)</span>
                </div>
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  <Clock className="w-3 h-3" />
                  <span>{new Date(c.last_seen).toLocaleDateString("fr-FR")}</span>
                </div>
              </div>

              <div className="mt-3 pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
                <span>{c.center_latitude.toFixed(3)}, {c.center_longitude.toFixed(3)}</span>
                <span>R={c.radius_km?.toFixed(1) ?? "—"} km</span>
              </div>
            </div>
          );
        })}
