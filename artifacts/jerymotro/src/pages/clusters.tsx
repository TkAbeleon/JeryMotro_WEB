...
export default function ClustersPage() {
  ...
  return (
    <div className="min-h-full bg-background px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
      ...
      ].map(k => { const Icon = k.icon; return <div key={k.label} className="rounded-xl border border-border/70 bg-card/70 p-4 shadow-sm transition-colors hover:bg-card sm:p-5"><div className={`w-8 h-8 rounded-lg ${k.bg} flex items-center justify-center mb-3`}><Icon className={`w-4 h-4 ${k.color}`} /></div><div className={`font-heading text-2xl font-bold ${k.color}`}>{k.value}</div><div className="text-xs text-muted-foreground mt-1">{k.label}</div></div>; })}</div>
      <section className="mb-1 rounded-xl border border-border/70 bg-card/45 p-3 shadow-sm sm:p-4"><div className="flex flex-wrap items-center gap-3"><div className="relative"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("clusters.search.placeholder")} className="h-9 w-56 pl-9 pr-3 rounded-md bg-card border border-border text-sm outline-none focus:ring-2 focus:ring-primary/30 transition-all" /></div><div className="flex flex-wrap gap-2">{filterButtons.map(f => <button key={f.key} onClick={() => setStatusFilter(f.key)} className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${statusFilter === f.key ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}>{f.label}</button>)}</div></div></section>
      {filtered.length === 0 ? <AsyncState type="empty" title={search || statusFilter !== "all" ? "Aucun cluster correspondant" : t("clusters.empty")} description={search || statusFilter !== "all" ? "Modifiez la recherche ou le filtre pour voir d’autres résultats." : undefined} /> : <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">{filtered.map(c => {
        ...
        return <div key={c.id} data-testid={`card-cluster-${c.id}`} className={`rounded-xl border border-border/70 bg-card/65 p-5 shadow-sm transition-all hover:bg-card hover:shadow-md ${statusLower === "active" ? "border-l-2 border-l-destructive" : ""}`}>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-border/50 pt-3 text-xs text-muted-foreground"><span>{c.center_latitude.toFixed(3)}, {c.center_longitude.toFixed(3)}</span><span>R={c.radius_km?.toFixed(1) ?? "—"} km</span></div>
        </div>;
      })}</div>}
    </div>
  );
