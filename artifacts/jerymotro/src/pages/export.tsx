...
export default function ExportPage() {
  ...
  if (!hasExtendedAccess) return <div className="p-4 sm:p-6"><div className="jm-export-card mx-auto max-w-2xl border border-border/70 bg-card/70 p-6 text-center shadow-sm sm:p-8"><Lock className="mx-auto mb-4 h-14 w-14 text-muted-foreground sm:h-16 sm:w-16" /><h1 className="mb-2 font-heading text-2xl font-bold">{t("export.title")}</h1><p className="mb-6 text-muted-foreground">{t("export.extendedAccessRequired")}</p><a href="/access-request" className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-6 py-3 font-semibold text-primary-foreground hover:opacity-90 sm:w-auto">{t("export.requestExtendedAccess")}</a></div></div>;

  return <div className="min-h-full bg-background px-4 py-5 sm:px-6 sm:py-7">
    <PageHeader title={t("export.title")} description={t("export.subtitle")} className="mb-6" />
    <div className="jm-export-card border border-border/70 bg-card/65 p-5 shadow-sm sm:p-6"><h2 className="mb-4 flex items-center gap-2 font-heading font-semibold"><CalendarIcon className="h-4 w-4" />{t("export.dateRange")}</h2><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div className="space-y-2"><label className="text-sm font-medium">{t("export.dateFrom")}</label><input type="date" value={formatDateForAPI(dateRange.from)} min="2020-01-01" max={formatDateForAPI(new Date())} onChange={e => { const [y,m,d] = e.target.value.split("-").map(Number); const from = new Date(y,m-1,d); setDateRange({ ...dateRange, from, to: dateRange.to < from ? from : dateRange.to }); }} className="jm-control-inset h-10 w-full border border-border bg-background px-3 text-sm" /></div><div className="space-y-2"><label className="text-sm font-medium">{t("export.dateTo")}</label><input type="date" value={formatDateForAPI(dateRange.to)} min={formatDateForAPI(dateRange.from)} max={formatDateForAPI(new Date())} onChange={e => { const [y,m,d] = e.target.value.split("-").map(Number); const to = new Date(y,m-1,d); setDateRange({ ...dateRange, to, from: dateRange.from > to ? to : dateRange.from }); }} className="jm-control-inset h-10 w-full border border-border bg-background px-3 text-sm" /></div></div><div className="mt-3 text-xs text-muted-foreground">{t("export.range")}: {format(dateRange.from,"dd/MM/yyyy")} - {format(dateRange.to,"dd/MM/yyyy")} <span className="ml-1 text-muted-foreground/60">({Math.ceil((dateRange.to.getTime()-dateRange.from.getTime())/86400000)} {t("export.days")})</span></div></div>
    <div className="jm-export-card border border-card-border bg-card p-5 sm:p-6"><h2 className="mb-4 font-heading font-semibold">{t("export.format")}</h2><div className="grid grid-cols-1 gap-3 sm:grid-cols-3">{[{id:"csv",icon:FileText,label:"CSV",desc:"Excel compatible"},{id:"json",icon:FileJson,label:"JSON",desc:"Developer friendly"},{id:"pdf",icon:FileText,label:"PDF",desc:"PDF professionnel"}].map(item => { const Icon = item.icon; return <button key={item.id} type="button" onClick={() => { setExportFormat(item.id as "csv"|"json"|"pdf"); setExportError(null); }} className={`jm-export-option min-h-28 border p-4 text-left shadow-sm ${exportFormat===item.id ? "border-primary bg-primary/5" : "border-border hover:border-border/60"}`}><Icon className={`mb-2 h-5 w-5 ${exportFormat===item.id ? "text-primary" : "text-muted-foreground"}`} /><div className="text-sm font-medium">{item.label}</div><div className="text-xs text-muted-foreground">{item.desc}</div></button>; })}</div></div>
    {exportError && <div role="alert" className="rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive shadow-sm">{exportError}</div>}
    <div className="jm-export-card border border-card-border bg-card p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h2 className="font-heading font-semibold">{t("export.ready")}</h2><p className="mt-1 text-sm text-muted-foreground">{detectionsQ.isLoading ? t("export.loading") : t("export.available", {count:detections.length})}</p></div><button type="button" onClick={handleExport} disabled={isExporting || detectionsQ.isLoading || detections.length===0} className="jm-export-action inline-flex min-h-11 w-full shrink-0 items-center justify-center gap-2 bg-primary px-6 py-3 font-semibold text-primary-foreground hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">{isExporting ? <><Loader2 className="h-4 w-4 animate-spin" />Génération du fichier…</> : <><Download className="h-4 w-4" />{t("export.export")}</>}</button></div></div>
  </div>;
}
