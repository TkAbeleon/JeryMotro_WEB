...
export default function DashboardPage() {
  ...
  const queryError = dailyQ.isError || detectionsQ.isError || clustersQ.isError;
  if (dailyQ.isLoading || detectionsQ.isLoading || clustersQ.isLoading) return <AsyncStateInline type="loading" title={t("common.loading")} description="Chargement des indicateurs et détections récentes…" />;
  ...
  return (
    <div className="min-h-full bg-background px-4 py-5 sm:px-6 sm:py-7 lg:px-8"><div className="mx-auto max-w-[1600px] space-y-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">{[
        ...
        { label: t("dashboard.kpi.alertsSent"), value: summary.critical_alerts, sub: t("dashboard.kpi.alertsSub"), icon: Bell, color: "text-warning", bg: "bg-warning/10" },
        ...
        {environmentRows.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {environmentRows.map((row, index) => (
              <div key={row.context} className="jm-environment-row rounded-lg border border-border/50 bg-background/35 p-3">
                ...
                <div className="jm-progress-track h-2 overflow-hidden rounded-full bg-secondary">
                  <div className="jm-progress-value h-full rounded-full bg-primary" style={{ width: `${Math.min(100, Math.max(0, row.percentage))}%`, opacity: Math.max(0.55, 1 - index * 0.08) }} />
                </div>
