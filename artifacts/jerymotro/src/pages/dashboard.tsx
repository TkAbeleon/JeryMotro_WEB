...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-7 p-4 sm:p-6">
      ...
      ].map(k => { const Icon = k.icon; return <div key={k.label} className="group rounded-xl border border-border/55 bg-card/55 p-4 transition-colors hover:bg-card sm:p-5"><div className="flex items-center justify-between"><div className={`flex h-8 w-8 items-center justify-center rounded-lg ${k.bg}`}><Icon className={`h-4 w-4 ${k.color}`} /></div><TrendingUp className="h-3.5 w-3.5 text-muted-foreground/25" /></div><div className={`mt-4 font-heading text-3xl font-semibold tracking-tight ${k.color}`}>{k.value}</div><div className="mt-1 text-sm font-medium">{k.label}</div><div className="mt-0.5 text-xs text-muted-foreground">{k.sub}</div></div>; })}</div>
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <div className="rounded-xl border border-border/55 bg-card/40 p-4 sm:p-5 lg:col-span-2"><div className="mb-5 flex items-center justify-between gap-4"><h2 className="font-heading text-sm font-semibold">{t("dashboard.chart.title")}</h2><div className="flex items-center gap-3 text-[10px] text-muted-foreground"><span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-primary/60" />{t("dashboard.chart.total")}</span><span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-destructive" />{t("dashboard.chart.highRisk")}</span></div></div><ResponsiveContainer width="100%" height={200}><AreaChart data={chartData}><defs><linearGradient id="gTotal" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(18 80% 50%)" stopOpacity={0.22} /><stop offset="95%" stopColor="hsl(18 80% 50%)" stopOpacity={0} /></linearGradient><linearGradient id="gHigh" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(0 84% 60%)" stopOpacity={0.18} /><stop offset="95%" stopColor="hsl(0 84% 60%)" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(150 15% 15%)" strokeOpacity={0.35} vertical={false} /><XAxis dataKey="fullDate" tick={{ fontSize: 10, fill: "hsl(150 8% 55%)" }} tickLine={false} axisLine={false} tickFormatter={value => { const parts = value.split('-'); return parts.length === 3 ? `${parts[2]}/${parts[1]}` : value; }} /><YAxis yAxisId="total" tick={{ fontSize: 10, fill: "hsl(150 8% 55%)" }} tickLine={false} axisLine={false} /><YAxis yAxisId="critical" orientation="right" tick={{ fontSize: 10, fill: "hsl(150 8% 55%)" }} tickLine={false} axisLine={false} domain={[0, criticalScaleMax]} /><Tooltip contentStyle={{ background: "hsl(var(--popover))", border: "1px solid hsl(var(--popover-border))", borderRadius: 10, fontSize: 12 }} labelFormatter={value => { const parts = value.split('-'); if (parts.length !== 3) return value; const localDate = new Date(parseInt(parts[0], 10), parseInt(parts[1], 10) - 1, parseInt(parts[2], 10)); const langCode = lang === "mg" ? "mg-MG" : lang === "fr" ? "fr-FR" : "en-US"; const formatted = new Intl.DateTimeFormat(langCode, { weekday: "long", day: "numeric", month: "long" }).format(localDate); return formatted.charAt(0).toUpperCase() + formatted.slice(1); }} /><Area type="monotone" dataKey="total" yAxisId="total" stroke="hsl(18 80% 50%)" strokeWidth={2} fill="url(#gTotal)" name={t("dashboard.chart.total")} /><Area type="monotone" dataKey="high" yAxisId="critical" stroke="hsl(0 84% 60%)" strokeWidth={2} fill="url(#gHigh)" name={t("dashboard.chart.highRisk")} /></AreaChart></ResponsiveContainer></div>
        <div className="rounded-xl border border-border/55 bg-card/40 p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><h2 className="font-heading text-sm font-semibold">{t("dashboard.regions.title")}</h2><span className="text-[10px] text-muted-foreground">{t("dashboard.regions.updated")}</span></div>{summary.regions_affected_today.length === 0 ? <AsyncStateInline type="empty" title="Aucune région signalée" description="Aucune détection régionale n'est disponible pour cette période." /> : <div className="space-y-1">{summary.regions_affected_today.map((region, i) => { const count = detectionsData.detections?.filter(d => d.region === region).length || 0; return <div key={region} className="flex items-center gap-3 rounded-lg px-2 py-2.5 transition-colors hover:bg-muted/35"><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${i === 0 ? "bg-destructive" : i === 1 ? "bg-primary" : "bg-[#f59e0b]"}`} /><span className="min-w-0 flex-1 truncate text-sm">{region}</span><span className="text-xs text-muted-foreground">{count} {t("dashboard.regions.fires")}</span></div>; })}</div>}</div>
      </div>
