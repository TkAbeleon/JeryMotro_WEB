...
export default function SubscriptionsPage() {
  ...
  return (
    <div className="min-h-full w-full bg-background px-4 py-5 sm:px-6 sm:py-7">
      <header className="border-b border-border/60 pb-5"><h1 className="font-heading text-2xl font-semibold tracking-tight">{t("subscriptions.title")}</h1><p className="mt-1 text-sm text-muted-foreground">{t("subscriptions.subtitle")}</p></header>
      {isPremium && <div className="flex flex-col gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/20"><Shield className="h-5 w-5 text-primary" /></div><div className="min-w-0"><div className="text-sm font-medium">{t("subscriptions.plan.premium")} actif</div><div className="text-xs text-muted-foreground">Toutes les fonctionnalités sont débloquées.</div></div></div><span className="w-fit shrink-0 rounded-full bg-primary/15 px-2 py-1 text-xs font-medium text-primary">{t("common.active")}</span></div>}
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
        <div className={`rounded-xl border bg-card/70 p-5 shadow-sm sm:p-6 ${!isPremium ? "border-primary/30" : "border-card-border"}`}>
        ...
        <div className={`rounded-xl border p-5 shadow-sm sm:p-6 ${isPremium ? "border-primary/50 bg-primary/5" : "border-border bg-card"}`}>
      ...
      <div className="overflow-hidden rounded-xl border border-border/70 bg-card/60 shadow-sm">
        <div className="border-b border-border/60 bg-background/20 p-4"><h2 className="font-heading font-semibold">Comparaison détaillée</h2></div>
        <div className="w-full overflow-x-auto"><div className="min-w-[500px]"><div className="grid grid-cols-3 border-b border-border/60 bg-muted/20 px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground"><span>Fonctionnalité</span><span className="text-center">{t("subscriptions.plan.free")}</span><span className="text-center">{t("subscriptions.plan.premium")}</span></div><div className="divide-y divide-border">{[
