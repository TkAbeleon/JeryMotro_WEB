    ...
    <div className="pointer-events-none absolute inset-0 z-10">
      <div className="pointer-events-auto absolute inset-x-3 top-3 sm:inset-x-4 sm:top-4">
        <div className="relative w-[min(420px,42vw)]">
          <form onSubmit={e => { e.preventDefault(); searchLocation(); }} className="flex h-11 items-center gap-2 rounded-xl border border-border/70 bg-card/95 px-3 shadow-lg backdrop-blur"><Search className="h-4 w-4 shrink-0 text-muted-foreground" /><input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={t("map.search.placeholder")} className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground/60" />{searching ? <Loader2 className="h-4 w-4 shrink-0 animate-spin text-primary" /> : searchQuery && <button type="button" onClick={() => { setSearchQuery(""); setSearchResults([]); }} aria-label={t("common.close")} className="rounded-lg p-1 text-muted-foreground hover:bg-secondary"><X className="h-4 w-4" /></button>}</form>
        ...
        <button type="button" onClick={() => setSearchOpen(true)} aria-label={t("map.search.placeholder")} className="absolute left-1/2 top-0 flex h-11 w-11 -translate-x-1/2 items-center justify-center rounded-xl border border-border bg-card/95 shadow-lg backdrop-blur md:hidden"><Search className="h-[18px] w-[18px]" /></button>
        <div className="absolute right-0 top-0 flex items-center gap-2"><button type="button" onClick={() => setControlsOpen(true)} aria-expanded={controlsOpen} className="relative flex h-11 items-center gap-2 rounded-xl border border-border bg-card/95 px-3.5 text-sm font-semibold shadow-lg backdrop-blur sm:px-4"><SlidersHorizontal className="h-[17px] w-[17px]" /><span className="hidden sm:inline">{t("map.filter.title")}</span>{activeFiltersCount > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-bold text-primary-foreground">{activeFiltersCount}</span>}</button><button type="button" onClick={locateMe} disabled={locating} aria-label={t("map.geolocation.button" as any)} className="flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-card/95 shadow-lg backdrop-blur disabled:opacity-50">{locating ? <Loader2 className="h-[17px] w-[17px] animate-spin text-primary" /> : <LocateFixed className="h-[17px] w-[17px] text-primary" />}</button></div>
      </div>

      <div className="pointer-events-auto absolute bottom-4 left-3 right-3 flex justify-center sm:bottom-5 sm:left-4 sm:right-4"><div className="rounded-xl border border-border/80 bg-card/92 px-3.5 py-2.5 shadow-lg backdrop-blur sm:px-4"><div className="flex items-center gap-3 text-xs sm:gap-4 sm:text-sm"><span><b>{filtered.length}</b> <span className="text-muted-foreground">{t("map.stats.total")}</span></span><span className="h-3.5 w-px bg-border" /><span className="flex items-center gap-1.5"><i className="h-1.5 w-1.5 rounded-full bg-destructive" /><b>{criticalCount}</b> <span className="text-muted-foreground">{t("map.stats.critical")}</span></span><span className="hidden h-3.5 w-px bg-border sm:block" /><span className="hidden text-muted-foreground sm:inline">{period === "custom" ? `${format(dateRange.from, "dd/MM")} – ${format(dateRange.to, "dd/MM")}` : periodLabel(period)}</span></div></div></div>
      ...
      {controlsOpen && <><button type="button" aria-label={t("common.close")} onClick={() => setControlsOpen(false)} className="pointer-events-auto absolute inset-0 bg-black/25 backdrop-blur-[1px] lg:bg-transparent lg:backdrop-blur-none" /><aside className="pointer-events-auto absolute bottom-3 right-3 top-16 flex w-[min(380px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-border bg-card/98 shadow-2xl backdrop-blur-xl sm:bottom-4 sm:right-4 sm:top-20">
        ...
        {tab === "list" ? <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3 sm:px-4">{visibleFires.length === 0 ? <AsyncStateInline type="empty" title={t("map.empty" as any)} description="Aucune détection ne correspond aux filtres ou à la zone visible." /> : visibleFires.map(fire => <button key={fire.id} type="button" onMouseEnter={() => setHoveredFireId(fire.id)} onMouseLeave={() => setHoveredFireId(null)} onClick={() => setTarget({ lat: fire.lat, lng: fire.lng, zoom: 14 })} className={`group w-full border-b border-border/60 px-2 py-3 text-left transition last:border-b-0 ${hoveredFireId === fire.id ? "bg-primary/5" : "hover:bg-secondary/45"}`}><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: getRiskColor(fire.risk) }} /><span className="min-w-0 flex-1 truncate text-sm font-semibold">{fire.region}</span><span className="text-[10px] font-bold uppercase" style={{ color: getRiskColor(fire.risk) }}>{fire.risk.toFixed(2)}</span></div><div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground"><span>{fire.sourceRaw || fire.source}</span><span>{format(fire.detectedAt, "dd/MM HH:mm")}</span></div></button>)}</div> : <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4"><div className="space-y-3.5 pt-1">
          <section className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.style")}</div><div className="grid grid-cols-3 gap-1 rounded-xl bg-secondary/70 p-1">{(["satellite", "roadmap", "dark"] as MapStyle[]).map(style => <button key={style} type="button" onClick={() => setMapStyle(style)} className={`h-9 rounded-lg text-[11px] font-semibold transition ${mapStyle === style ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>{t(`map.filter.style.${style}` as any).replace(" (Google)", "")}</button>)}</div></section>
