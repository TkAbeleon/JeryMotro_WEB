  ...
  return <div className={`relative isolate ${mapHeightClass} min-h-0 w-full overflow-hidden bg-background`>
    ...
    <div className="pointer-events-none absolute inset-0 z-10">
      <div className="pointer-events-auto absolute inset-x-3 top-3 sm:inset-x-4 sm:top-4">
        ...
        <button type="button" onClick={() => setSearchOpen(true)} aria-label={t("map.search.placeholder")} className="jm-map-float absolute left-1/2 top-0 flex h-11 w-11 -translate-x-1/2 items-center justify-center rounded-2xl border border-border bg-card/95 backdrop-blur md:hidden"><Search className="h-[18px] w-[18px]" /></button>
        <div className="absolute right-0 top-0 flex items-center gap-2"><button type="button" onClick={() => setControlsOpen(true)} aria-expanded={controlsOpen} className="jm-map-float relative flex h-11 items-center gap-2 rounded-2xl border border-border bg-card/95 px-3.5 text-sm font-semibold backdrop-blur sm:px-4"><SlidersHorizontal className="h-[17px] w-[17px]" /><span className="hidden sm:inline">{t("map.filter.title")}</span>{activeFiltersCount > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-bold text-primary-foreground">{activeFiltersCount}</span>}</button><button type="button" onClick={locateMe} disabled={locating} aria-label={t("map.geolocation.button" as any)} className="jm-map-float flex h-11 w-11 items-center justify-center rounded-2xl border border-border bg-card/95 backdrop-blur disabled:opacity-50">{locating ? <Loader2 className="h-[17px] w-[17px] animate-spin text-primary" /> : <LocateFixed className="h-[17px] w-[17px] text-primary" />}</button></div>
      </div>

      <div className="pointer-events-auto absolute bottom-4 left-3 right-3 flex justify-center sm:bottom-5 sm:left-4 sm:right-4"><div className="jm-map-float rounded-2xl border border-border/80 bg-card/92 px-3.5 py-2.5 backdrop-blur sm:px-4"><div className="flex items-center gap-3 text-xs sm:gap-4 sm:text-sm"><span><b>{filtered.length}</b> <span className="text-muted-foreground">{t("map.stats.total")}</span></span><span className="h-3.5 w-px bg-border" /><span className="flex items-center gap-1.5"><i className="h-1.5 w-1.5 rounded-full bg-destructive" /><b>{criticalCount}</b> <span className="text-muted-foreground">{t("map.stats.critical")}</span></span><span className="hidden h-3.5 w-px bg-border sm:block" /><span className="hidden text-muted-foreground sm:inline">{period === "custom" ? `${format(dateRange.from, "dd/MM")} – ${format(dateRange.to, "dd/MM")}` : periodLabel(period)}</span></div></div></div>

      {searchOpen && <div className="pointer-events-auto absolute inset-0 bg-black/35 p-3 backdrop-blur-[2px] sm:p-6 md:hidden"><div className="jm-map-panel mx-auto mt-16 w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-card shadow-2xl"><form onSubmit={e => { e.preventDefault(); searchLocation(); }} className="flex items-center gap-2 border-b border-border p-3"><Search className="ml-1 h-5 w-5 text-muted-foreground" /><input autoFocus value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={t("map.search.placeholder")} className="h-11 min-w-0 flex-1 bg-transparent px-2 text-sm outline-none" /><button type="button" onClick={() => { setSearchOpen(false); setSearchResults([]); setSearchQuery(""); }} aria-label={t("common.close")} className="flex h-10 w-10 items-center justify-center rounded-xl hover:bg-secondary"><X className="h-5 w-5" /></button></form><div className="max-h-[55vh] overflow-y-auto p-2">{searching && <div className="flex items-center justify-center gap-2 p-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />{t("map.search.searching" as any)}</div>}{!searching && searchResults.map((result, index) => <button key={`${result.lat}-${index}`} type="button" onClick={() => selectSearchResult(result)} className="jm-map-result flex w-full items-start gap-3 rounded-2xl p-3 text-left transition hover:bg-secondary"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span className="text-sm">{result.formatted_address}</span></button>)}</div></div></div>}
      ...
      {controlsOpen && <><button type="button" aria-label={t("common.close")} onClick={() => setControlsOpen(false)} className="pointer-events-auto absolute inset-0 bg-black/25 backdrop-blur-[1px] lg:bg-transparent lg:backdrop-blur-none" /><aside className="jm-map-panel pointer-events-auto absolute bottom-3 right-3 top-16 flex w-[min(380px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-border bg-card/98 shadow-2xl backdrop-blur-xl sm:bottom-4 sm:right-4 sm:top-20">
        <div className="flex items-center justify-between border-b border-border bg-background/25 px-4 py-3.5 sm:px-5"><div><div className="font-heading text-sm font-bold sm:text-base">{t("map.filter.title")}</div><div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{filtered.length} {t("map.stats.total")} · données actives</div></div><button ref={closeRef} type="button" onClick={() => setControlsOpen(false)} aria-label={t("common.close")} className="flex h-10 w-10 items-center justify-center rounded-xl text-muted-foreground hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><X className="h-5 w-5" /></button></div>
