    ...
    <div className="pointer-events-none absolute inset-0 z-10">
      ...
      {controlsOpen && <><button type="button" aria-label={t("common.close")} onClick={() => setControlsOpen(false)} className="pointer-events-auto absolute inset-0 bg-black/25 backdrop-blur-[1px] lg:bg-transparent lg:backdrop-blur-none" /><aside className="jm-map-panel pointer-events-auto absolute bottom-3 right-3 top-16 flex w-[min(380px,calc(100vw-24px))] flex-col overflow-hidden rounded-3xl border border-border bg-card/98 shadow-2xl backdrop-blur-xl sm:bottom-4 sm:right-4 sm:top-20">
        ...
        {tab === "list" ? <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3 sm:px-4">{visibleFires.length === 0 ? <AsyncStateInline type="empty" title={t("map.empty" as any)} description="Aucune détection ne correspond aux filtres ou à la zone visible." /> : visibleFires.map(fire => <button key={fire.id} type="button" onMouseEnter={() => setHoveredFireId(fire.id)} onMouseLeave={() => setHoveredFireId(null)} onClick={() => setTarget({ lat: fire.lat, lng: fire.lng, zoom: 14 })} className={`jm-map-list-item group w-full border-b border-border/60 px-2 py-3 text-left transition last:border-b-0 ${hoveredFireId === fire.id ? "jm-map-list-item-active bg-primary/5" : "hover:bg-secondary/45"}`}><div className="flex items-center gap-2"><span className="jm-map-risk-dot h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: getRiskColor(fire.risk) }} /><span className="min-w-0 flex-1 truncate text-sm font-semibold">{fire.region}</span><span className="text-[10px] font-bold uppercase" style={{ color: getRiskColor(fire.risk) }}>{fire.risk.toFixed(2)}</span></div><div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground"><span>{fire.sourceRaw || fire.source}</span><span>{format(fire.detectedAt, "dd/MM HH:mm")}</span></div></button>)}</div> : <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4"><div className="space-y-3.5 pt-1">
          <section className="jm-map-panel-section rounded-2xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.style")}</div><div className="grid grid-cols-3 gap-1 rounded-xl bg-secondary/70 p-1">{(["satellite", "roadmap", "dark"] as MapStyle[]).map(style => <button key={style} type="button" onClick={() => setMapStyle(style)} className={`h-9 rounded-lg text-[11px] font-semibold transition ${mapStyle === style ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>{t(`map.filter.style.${style}` as any).replace(" (Google)", "")}</button>)}</div></section>
          <section className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.period")}</div><div className="grid grid-cols-3 gap-1.5">{PERIODS.map(p => <button key={p} type="button" onClick={() => setPeriod(p)} className={`min-h-10 rounded-xl border px-2 text-[11px] font-semibold transition ${period === p ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-secondary"}`}>{periodLabel(p)}</button>)}</div></section>
          ...
          <section className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.risk")}</div><div className="grid grid-cols-2 gap-1.5">{RISK_LEVELS.map(({ key, color }) => { const checked = selectedRisks.has(key); return <button key={key} type="button" onClick={() => toggleRisk(key)} className={`flex min-h-10 items-center gap-2 rounded-xl border px-3 text-left text-[11px] font-semibold transition ${checked ? "border-border bg-secondary" : "border-border/70 text-muted-foreground opacity-60"}`}><span className="flex h-5 w-5 items-center justify-center rounded-md border-2" style={{ borderColor: color, background: checked ? color : "transparent" }}>{checked && <Check className="h-3 w-3 text-white" />}</span><span className="truncate">{riskLabel(key)}</span></button>; })}</div></section>
          <section className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.region")}</div><div className="relative"><select value={selectedRegion} onChange={e => setSelectedRegion(e.target.value)} className="h-10 w-full appearance-none rounded-xl border border-border bg-background px-3 pr-9 text-sm outline-none focus:ring-2 focus:ring-primary"><option value="all">{t("map.filter.region.all")}</option>{MADAGASCAR_REGIONS.map(region => <option key={region.name} value={region.name}>{region.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /></div></section>
          <section className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.source")}</div><div className="grid grid-cols-3 gap-1.5">{["all", "MODIS", "VIIRS"].map(source => <button key={source} type="button" onClick={() => setSelectedSource(source)} className={`min-h-10 rounded-xl border text-[11px] font-semibold transition ${selectedSource === source ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-secondary"}`}>{source === "all" ? t("map.filter.source.all") : source}</button>)}</div></section>
          <section className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.context")}</div><div className="grid grid-cols-1 gap-1.5">{[{ key: "all", label: t("map.filter.context.all") }, { key: "pending", label: t("map.filter.context.pending") }, ...WORLDCOVER_CONTEXTS.map(context => ({ key: context.key, label: t(context.i18n as any) }))].map(context => <button key={context.key} type="button" onClick={() => setSelectedContext(context.key)} className={`min-h-10 rounded-xl border px-3 py-2 text-left text-[11px] font-semibold transition ${selectedContext === context.key ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-secondary"}`}>{context.label}</button>)}</div></section>
