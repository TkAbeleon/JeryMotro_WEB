  ...
  return <div className={`relative isolate ${mapHeightClass} min-h-0 w-full overflow-hidden bg-background`>
    ...
    <div className="pointer-events-none absolute inset-0 z-10">
      <div className="pointer-events-auto absolute inset-x-3 top-3 sm:inset-x-4 sm:top-4">
        ...
        <button type="button" onClick={() => setSearchOpen(true)} aria-label={t("map.search.placeholder")} className="jm-map-float absolute left-1/2 top-0 flex h-11 w-11 -translate-x-1/2 items-center justify-center rounded-2xl backdrop-blur md:hidden"><Search className="h-[18px] w-[18px]" /></button>
        <div className="absolute right-0 top-0 flex items-center gap-2"><button type="button" onClick={() => setControlsOpen(true)} aria-expanded={controlsOpen} className="jm-map-float relative flex h-11 items-center gap-2 rounded-2xl px-3.5 text-sm font-semibold backdrop-blur sm:px-4"><SlidersHorizontal className="h-[17px] w-[17px]" /><span className="hidden sm:inline">{t("map.filter.title")}</span>{activeFiltersCount > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-bold text-primary-foreground">{activeFiltersCount}</span>}</button><button type="button" onClick={locateMe} disabled={locating} aria-label={t("map.geolocation.button" as any)} className="jm-map-float flex h-11 w-11 items-center justify-center rounded-2xl backdrop-blur disabled:opacity-50">{locating ? <Loader2 className="h-[17px] w-[17px] animate-spin text-primary" /> : <LocateFixed className="h-[17px] w-[17px] text-primary" />}</button></div>
      </div>

      <div className="pointer-events-auto absolute bottom-4 left-3 right-3 flex justify-center sm:bottom-5 sm:left-4 sm:right-4"><div className="jm-map-float rounded-2xl px-3.5 py-2.5 backdrop-blur sm:px-4"><div className="flex items-center gap-3 text-xs sm:gap-4 sm:text-sm"><span><b>{filtered.length}</b> <span className="text-muted-foreground">{t("map.stats.total")}</span></span><span className="h-3.5 w-px bg-border" /><span className="flex items-center gap-1.5"><i className="h-1.5 w-1.5 rounded-full bg-destructive" /><b>{criticalCount}</b> <span className="text-muted-foreground">{t("map.stats.critical")}</span></span><span className="hidden h-3.5 w-px bg-border sm:block" /><span className="hidden text-muted-foreground sm:inline">{period === "custom" ? `${format(dateRange.from, "dd/MM")} – ${format(dateRange.to, "dd/MM")}` : periodLabel(period)}</span></div></div></div>

      {searchOpen && <div className="pointer-events-auto absolute inset-0 bg-black/35 p-3 backdrop-blur-[2px] sm:p-6 md:hidden"><div className="jm-map-panel mx-auto mt-16 w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-card shadow-2xl"><form onSubmit={e => { e.preventDefault(); searchLocation(); }} className="flex items-center gap-2 border-b border-border/40 p-3"><Search className="ml-1 h-5 w-5 text-muted-foreground" /><input autoFocus value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={t("map.search.placeholder")} className="h-11 min-w-0 flex-1 bg-transparent px-2 text-sm outline-none" /><button type="button" onClick={() => { setSearchOpen(false); setSearchResults([]); setSearchQuery(""); }} aria-label={t("common.close")} className="flex h-10 w-10 items-center justify-center rounded-xl hover:bg-secondary"><X className="h-5 w-5" /></button></form><div className="max-h-[55vh] overflow-y-auto p-2">{searching && <div className="flex items-center justify-center gap-2 p-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />{t("map.search.searching" as any)}</div>}{!searching && searchResults.map((result, index) => <button key={`${result.lat}-${index}`} type="button" onClick={() => selectSearchResult(result)} className="jm-map-result flex w-full items-start gap-3 rounded-2xl p-3 text-left transition hover:bg-secondary"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span className="text-sm">{result.formatted_address}</span></button>)}</div></div></div>}

      {controlsOpen && <><button type="button" aria-label={t("common.close")} onClick={() => setControlsOpen(false)} className="pointer-events-auto absolute inset-0 bg-black/25 backdrop-blur-[1px] lg:bg-transparent lg:backdrop-blur-none" /><aside className="jm-map-panel pointer-events-auto absolute bottom-3 right-3 top-16 flex w-[min(380px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-border bg-card/98 shadow-2xl backdrop-blur-xl sm:bottom-4 sm:right-4 sm:top-20">
        <div className="flex items-center justify-between border-b border-border/40 bg-background/25 px-4 py-3.5 sm:px-5"><div><div className="font-heading text-sm font-bold sm:text-base">{t("map.filter.title")}</div><div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{filtered.length} {t("map.stats.total")} · données actives</div></div><button ref={closeRef} type="button" onClick={() => setControlsOpen(false)} aria-label={t("common.close")} className="jm-map-float flex h-10 w-10 items-center justify-center rounded-2xl text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><X className="h-5 w-5" /></button></div>
        <div className="border-b border-border bg-secondary/20 p-3 sm:p-4"><div className="grid grid-cols-2 rounded-lg bg-secondary/70 p-1"><button type="button" onClick={() => setTab("list")} className={`flex h-10 items-center justify-center gap-2 rounded-lg text-xs font-semibold transition ${tab === "list" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}><List className="h-4 w-4" />{t("map.sidebar.tab.list") || "Liste"}</button><button type="button" onClick={() => setTab("filters")} className={`flex h-10 items-center justify-center gap-2 rounded-lg text-xs font-semibold transition ${tab === "filters" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}><Filter className="h-4 w-4" />{t("map.sidebar.tab.filters") || "Filtres"}{activeFiltersCount > 0 && <span className="rounded-full bg-primary/10 px-1.5 text-[10px] text-primary">{activeFiltersCount}</span>}</button></div></div>
        {tab === "list" ? <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3 sm:px-4">{visibleFires.length === 0 ? <AsyncStateInline type="empty" title={t("map.empty" as any)} description="Aucune détection ne correspond aux filtres ou à la zone visible." /> : visibleFires.map(fire => <button key={fire.id} type="button" onMouseEnter={() => setHoveredFireId(fire.id)} onMouseLeave={() => setHoveredFireId(null)} onClick={() => setTarget({ lat: fire.lat, lng: fire.lng, zoom: 14 })} className={`jm-map-list-item group w-full border-b border-border/60 px-2 py-3 text-left transition last:border-b-0 ${hoveredFireId === fire.id ? "jm-map-list-item-active bg-primary/5" : "hover:bg-secondary/45"}`}><div className="flex items-center gap-2"><span className="jm-map-risk-dot h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: getRiskColor(fire.risk) }} /><span className="min-w-0 flex-1 truncate text-sm font-semibold">{fire.region}</span><span className="text-[10px] font-bold uppercase" style={{ color: getRiskColor(fire.risk) }}>{fire.risk.toFixed(2)}</span></div><div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground"><span>{fire.sourceRaw || fire.source}</span><span>{format(fire.detectedAt, "dd/MM HH:mm")}</span></div></button>)}</div> : <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4"><div className="space-y-3.5 pt-1">
          <section className="jm-map-panel-section rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.style")}</div><div className="grid grid-cols-3 gap-1 rounded-lg bg-secondary/70 p-1.5">{(["satellite", "roadmap", "dark"] as MapStyle[]).map(style => <button key={style} type="button" onClick={() => setMapStyle(style)} className={`h-9 rounded-md text-[11px] font-semibold transition ${mapStyle === style ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>{t(`map.filter.style.${style}` as any).replace(" (Google)", "")}</button>)}</div></section>
          <section className="jm-map-panel-section rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.period")}</div><div className="grid grid-cols-3 gap-1.5">{PERIODS.map(p => <button key={p} type="button" onClick={() => setPeriod(p)} className={`min-h-10 rounded-lg border px-2 text-[11px] font-semibold transition ${period === p ? "jm-filter-active border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-secondary"}`}>{periodLabel(p)}</button>)}</div></section>
          {period === "custom" && <section className="grid grid-cols-2 gap-2"><label className="text-[11px] text-muted-foreground">{t("export.dateFrom" as any)}<input type="date" value={formatDateForAPI(dateRange.from)} max={formatDateForAPI(new Date())} onChange={e => { const d = new Date(`${e.target.value}T00:00:00`); setDateRange(v => ({ from: d, to: v.to < d ? d : v.to })); }} className="jm-control-inset mt-1.5 h-10 w-full rounded-xl border border-border bg-background px-2 text-xs text-foreground focus:ring-0" /></label><label className="text-[11px] text-muted-foreground">{t("export.dateTo" as any)}<input type="date" value={formatDateForAPI(dateRange.to)} min={formatDateForAPI(dateRange.from)} max={formatDateForAPI(new Date())} onChange={e => { const d = new Date(`${e.target.value}T00:00:00`); setDateRange(v => ({ from: v.from > d ? d : v.from, to: d })); }} className="jm-control-inset mt-1.5 h-10 w-full rounded-xl border border-border bg-background px-2 text-xs text-foreground focus:ring-0" /></label></section>}
          <section className="jm-map-panel-section rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.risk")}</div><div className="grid grid-cols-2 gap-1.5">{RISK_LEVELS.map(({ key, color }) => { const checked = selectedRisks.has(key); return <button key={key} type="button" onClick={() => toggleRisk(key)} className={`jm-pill-depth flex min-h-10 items-center gap-2 rounded-xl border px-3 text-left text-[11px] font-semibold transition ${checked ? "border-border bg-secondary" : "border-border/70 text-muted-foreground opacity-60"}`}><span className="flex h-5 w-5 items-center justify-center rounded-md border-2" style={{ borderColor: color, background: checked ? color : "transparent" }}>{checked && <Check className="h-3 w-3 text-white" />}</span><span className="truncate">{riskLabel(key)}</span></button>; })}</div></section>
          <section className="jm-map-panel-section rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.region")}</div><div className="relative"><select value={selectedRegion} onChange={e => setSelectedRegion(e.target.value)} className="jm-control-inset h-10 w-full appearance-none rounded-xl border border-border bg-background px-3 pr-9 text-sm outline-none focus:ring-0"><option value="all">{t("map.filter.region.all")}</option>{MADAGASCAR_REGIONS.map(region => <option key={region.name} value={region.name}>{region.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /></div></section>
          <section className="jm-map-panel-section rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.source")}</div><div className="grid grid-cols-3 gap-1.5">{["all", "MODIS", "VIIRS"].map(source => <button key={source} type="button" onClick={() => setSelectedSource(source)} className={`jm-pill-depth min-h-10 rounded-xl border text-[11px] font-semibold transition ${selectedSource === source ? "jm-filter-active border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-secondary"}`}>{source === "all" ? t("map.filter.source.all") : source}</button>)}</div></section>
          <section className="jm-map-panel-section rounded-xl border border-border/70 bg-background/25 p-3"><div className="mb-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("map.filter.context")}</div><div className="grid grid-cols-1 gap-1.5">{[{ key: "all", label: t("map.filter.context.all") }, { key: "pending", label: t("map.filter.context.pending") }, ...WORLDCOVER_CONTEXTS.map(context => ({ key: context.key, label: t(context.i18n as any) }))].map(context => <button key={context.key} type="button" onClick={() => setSelectedContext(context.key)} className={`jm-pill-depth min-h-10 rounded-xl border px-3 py-2 text-left text-[11px] font-semibold transition ${selectedContext === context.key ? "jm-filter-active border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-secondary"}`}>{context.label}</button>)}</div></section>
          <section className="jm-map-panel-section flex items-center justify-between rounded-2xl border border-border bg-secondary/40 p-3"><div className="flex items-center gap-2"><Layers className="h-4 w-4 text-muted-foreground" /><span className="text-xs font-semibold">{t("map.legend")}</span></div><div className="flex items-center gap-3 text-[10px] text-muted-foreground">{RISK_LEVELS.map(({ key, color }) => <span key={key} className="flex items-center gap-1"><i className="h-2 w-2 rounded-full" style={{ background: color }} />{riskLabel(key)}</span>)}</div></section>
          <button type="button" onClick={resetFilters} className="jm-button-depth flex min-h-10 w-full items-center justify-center rounded-xl border border-border text-xs font-semibold text-muted-foreground transition hover:bg-secondary hover:text-foreground">{t("common.reset")}</button>
        </div></div>}
      </aside></>}
    </div>
  </div>;
