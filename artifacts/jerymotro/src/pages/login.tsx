...
export default function LoginPage() {
  ...
  return <div className="jm-auth-page min-h-screen bg-background flex">
    ...
    <div className="flex-1 flex items-center justify-center px-5 py-7 sm:p-8"><div className="jm-auth-column w-full max-w-[420px]">
      ...
      {mode === "password" ? <><div className="jm-auth-info mb-6 border border-primary/20 bg-primary/5 p-3 shadow-sm text-xs text-muted-foreground"><span className="text-primary font-medium">{t("auth.login.demo")}:</span> demo@jerymotro.mg / demo1234</div>{error && <div className="jm-auth-info mb-4 border border-destructive/20 bg-destructive/5 p-3 text-sm text-destructive shadow-sm">{error}</div>}<Form {...form}><form onSubmit={e => { e.preventDefault(); form.handleSubmit(onSubmit)(e); }} className="space-y-4"><FormField control={form.control} name="email" render={({ field }) => <FormItem><FormLabel>{t("auth.login.email")}</FormLabel><FormControl><input {...field} type="email" data-testid="input-email" placeholder={t("auth.login.emailPlaceholder")} className="jm-auth-control w-full h-10 px-3 bg-secondary border border-input text-sm focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none" /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="password" render={({ field }) => <FormItem><FormLabel>{t("auth.login.password")}</FormLabel><FormControl><div className="relative"><input {...field} type={showPassword ? "text" : "password"} data-testid="input-password" placeholder="••••••••" className="jm-auth-control w-full h-10 px-3 pr-10 bg-secondary border border-input text-sm focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none" /><button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div></FormControl><FormMessage /></FormItem>} /><button type="submit" data-testid="button-submit" disabled={loginMutation.isPending} className="jm-auth-action w-full h-10 bg-primary text-primary-foreground font-semibold text-sm hover:opacity-90 disabled:opacity-50">{loginMutation.isPending ? t("auth.login.submitting") : t("auth.login.submit")}</button></form></Form></> : <div className="space-y-5">
        ...
        {otpStep === "request" ? <>
          ...
          <button type="button" onClick={verifyOtp} disabled={verifyOtpMutation.isPending || !otpCodeValid} className="jm-auth-action w-full h-10 bg-primary text-primary-foreground font-semibold text-sm hover:opacity-90 disabled:opacity-50">{verifyOtpMutation.isPending ? "Vérification..." : "Se connecter"}</button>
