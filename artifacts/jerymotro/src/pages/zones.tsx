  ...
  if (!hasExtendedAccess) {
    return (
      <div className="p-4 sm:p-6 h-full flex flex-col items-center justify-center gap-6 text-center">
        ...
        <div>
          <h2 className="font-heading text-xl font-bold mb-2">{t("zones.extended.title")}</h2>
          <p className="text-muted-foreground text-sm max-w-sm">{t("zones.extended.desc")}</p>
        </div>
        <a
          href="/access-request"
    ...
    <div className="min-h-full bg-background p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row">
          ...
          <div className="flex-1 order-2 lg:order-1">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {zones.map(z => (
                <div key={z.id} data-testid={`card-zone-${z.id}`} className="jm-card-depth jm-zone-card group flex flex-col justify-between rounded-xl border border-border/70 bg-card/65 p-5 shadow-sm transition-colors hover:bg-card">
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      <div className="jm-zone-icon w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
          ...
          {/* Overview Google Map */}
          <div className="jm-zone-map sticky top-4 order-1 flex h-[350px] w-full flex-col gap-2.5 rounded-2xl border border-border/70 bg-card/65 p-3 sm:h-[400px] lg:order-2 lg:h-[480px] lg:w-[360px] xl:w-[420px]">
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider px-1">
              Carte des zones prioritaires (Google Maps)
            </div>
            <div className="relative z-0 flex-1 overflow-hidden rounded-xl border border-border/60 bg-background/20">
              ...
              <div className="absolute top-3 left-3 right-3 z-[500] flex flex-col overview-search-container">
                <form
                  ...
                  className="jm-control-inset flex items-center gap-1.5 bg-card/90 backdrop-blur-md border border-border rounded-xl pl-3 pr-2 py-1.5 h-10 w-full"
                >
                  ...
                  <button
                    ...
                    className="jm-button-depth bg-primary hover:opacity-90 disabled:opacity-50 text-primary-foreground text-xs font-semibold px-2.5 py-1 rounded-lg flex items-center justify-center h-7 transition-all flex-shrink-0"
                  >
                    {overviewSearching ? (
                ...
                {showOverviewDropdown && overviewSearchResults.length > 0 && (
                  <div className="jm-zone-modal mt-1.5 bg-card/95 backdrop-blur-md border border-border rounded-xl max-h-40 overflow-y-auto w-full py-1 divide-y divide-border/40 z-[1000] scrollbar-thin">
          ...
          <div className="jm-modal-shell jm-zone-modal w-full max-w-3xl max-h-[calc(100dvh-2rem)] overflow-y-auto border border-border/70 bg-card p-5 sm:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2 min-h-[300px] md:min-h-[400px]">
                ...
                <div className="flex-1 h-full rounded-xl overflow-hidden border border-border relative z-0">
                  ...
                  <div className="absolute top-2.5 left-2.5 right-[130px] z-[500] flex flex-col modal-search-container">
                    <form
                      ...
                      className="jm-control-inset flex items-center gap-1.5 bg-card/90 backdrop-blur-md border border-border rounded-xl pl-3 pr-2 py-1 h-8 w-full"
                    >
                      ...
                      <button
                        ...
                        className="jm-button-depth bg-primary hover:opacity-90 disabled:opacity-50 text-primary-foreground text-[10px] font-semibold px-2 py-0.5 rounded-lg flex items-center justify-center h-6 transition-all flex-shrink-0"
                      >
                        {modalSearching ? (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="jm-control-inset w-full h-10 px-3 rounded-xl bg-secondary border border-input text-sm outline-none focus:ring-0"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input value={form.radius_km} onChange={e => setForm(f => ({ ...f, radius_km: e.target.value }))} placeholder="25" required type="number" min="1" max="200" className="jm-control-inset w-full h-10 px-3 rounded-xl bg-secondary border border-input text-sm outline-none focus:ring-0" />
                    </div>
                    <div>
                      <label className="text-sm font-medium block mb-1.5">{t("zones.form.riskThreshold")}</label>
                      <input value={form.min_risk} onChange={e => setForm(f => ({ ...f, min_risk: e.target.value }))} placeholder="0.7" type="number" min="0" max="1" step="0.1" className="jm-control-inset w-full h-10 px-3 rounded-xl bg-secondary border border-input text-sm outline-none focus:ring-0" />
                    </div>
                  </div>
                  <div>
                    <label className="text-sm font-medium block mb-1.5">FRP Seuil (MW)</label>
                    <input value={form.min_frp} onChange={e => setForm(f => ({ ...f, min_frp: e.target.value }))} placeholder="50" type="number" min="0" className="jm-control-inset w-full h-10 px-3 rounded-xl bg-secondary border border-input text-sm outline-none focus:ring-0" />
                  </div>
                  <div>
                    <label className="text-sm font-medium block mb-1.5">{t("zones.form.aiPrompt")} ({t("common.optional")})</label>
                    <textarea value={form.custom_ai_prompt} onChange={e => setForm(f => ({ ...f, custom_ai_prompt: e.target.value }))} placeholder={t("zones.form.aiPromptPlaceholder")} rows={2} className="jm-control-inset w-full px-3 py-2 rounded-xl bg-secondary border border-input text-sm outline-none focus:ring-0 resize-none text-xs" />
                  </div>
                </div>
                <div className="flex gap-3 pt-4 border-t border-border mt-4">
                  <button type="button" onClick={() => setShowForm(false)} className="jm-button-depth flex-1 h-10 border border-border rounded-xl text-sm font-medium hover:bg-secondary transition-colors">
                    {t("common.cancel")}
                  </button>
                  <button type="submit" disabled={createMutation.isPending} className="jm-button-depth flex-1 h-10 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-50">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="jm-modal-shell jm-zone-delete-modal w-full max-w-sm max-h-[calc(100dvh-2rem)] overflow-y-auto border border-border/70 bg-card p-5 sm:p-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="jm-button-depth flex-1 h-10 bg-destructive text-destructive-foreground rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity"
              >
                {t("common.delete")}
              </button>
            </div>
