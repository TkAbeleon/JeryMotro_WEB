    ...
    <div className="min-h-full w-full bg-background px-4 py-5 sm:px-6 sm:py-7">
      ...
      <div className="jm-card-depth flex flex-col gap-4 rounded-xl border border-border/70 bg-card/65 p-5 shadow-sm sm:flex-row sm:items-start sm:gap-5 sm:p-6">
        ...
        <div className="min-w-0 flex-1 text-center sm:text-left">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-center gap-2 sm:justify-start"><span className={`jm-pill-depth rounded-full px-2 py-0.5 text-xs font-medium capitalize ${hasExtendedAccess ? "bg-primary/15 text-primary" : "bg-secondary text-muted-foreground"}`}>{profile?.role === "premium" ? "Accès étendu" : profile?.role === "admin" ? "Administrateur" : "Compte Standard"}</span><span className={`jm-pill-depth rounded-full px-2 py-0.5 text-xs ${profile?.is_active ? "bg-accent/15 text-accent" : "bg-destructive/15 text-destructive"}`}>{profile?.is_active ? t("profile.badge.active") : t("profile.badge.inactive")}</span></div>
        </div>
      </div>
      ...
      <div className="jm-profile-danger rounded-xl border border-destructive/25 bg-card/60 p-5 shadow-sm sm:p-6">
