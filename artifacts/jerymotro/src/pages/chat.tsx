...
export default function ChatPage() {
  ...
  return <section className="flex h-[calc(100dvh-58px)] min-h-0 flex-col bg-background"><header className="shrink-0 border-b border-border/60 bg-background/88 shadow-sm backdrop-blur-xl"><div className="mx-auto flex h-14 w-full max-w-4xl items-center justify-between px-4 sm:px-6"><div className="flex min-w-0 items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/9 text-primary"><Sparkles className="h-4 w-4" /></div><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-sm font-semibold tracking-tight">{t("chat.title")}</h1><span className="hidden items-center gap-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary/75 sm:inline-flex"><Zap className="h-2.5 w-2.5" />{t("chat.badge")}</span></div><p className="truncate text-[11px] text-muted-foreground">{t("chat.subtitle")}</p></div></div><button type="button" onClick={startNewConversation} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-border/50 px-2.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><Plus className="h-3.5 w-3.5" /><span className="hidden sm:inline">{t("chat.newConversation")}</span></button></div></header>
