...
mermaid.initialize({
  ...
  themeVariables: {
    primaryColor: "#eefbf3",
    primaryTextColor: "#143326",
    primaryBorderColor: "#b8dfc5",
    lineColor: "#7fa88f",
...
function CopyButton({ text, label, copiedLabel }: { text: string; label: string; copiedLabel: string }) {
  ...
  return (
    <button type="button" onClick={copy} title={copied ? copiedLabel : label} aria-label={copied ? copiedLabel : label} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-[11px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
...
export default function ChatPage() {
  ...
  const sendMessage = async (rawText: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: Date.now() + 1, role: "assistant", content: res.response, sources: res.sources, model: res.model_used ?? undefined, time_ms: res.response_time_ms ?? undefined, createdAt: Date.now() }]);
    } catch {
      setMessages((prev) => [...prev, { id: Date.now() + 1, role: "assistant", content: t("chat.errorMessage"), isError: true, retryText: text, createdAt: Date.now() }]);
    } finally {
  ...
  return (
    <section className="flex h-[calc(100dvh-58px)] min-h-0 flex-col bg-background">
      <header className="shrink-0 border-b border-border/45 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex h-14 w-full max-w-4xl items-center justify-between px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/9 text-primary">
              <Sparkles className="h-4 w-4" />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h1 className="truncate text-sm font-semibold tracking-tight">{t("chat.title")}</h1>
                <span className="hidden items-center gap-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary/75 sm:inline-flex"><Zap className="h-2.5 w-2.5" />{t("chat.badge")}</span>
              </div>
              <p className="truncate text-[11px] text-muted-foreground">{t("chat.subtitle")}</p>
            </div>
          </div>
          <button type="button" onClick={startNewConversation} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
      ...
      <div ref={messagesContainerRef} onScroll={() => { const atBottom = isNearBottom(); setIsAtBottom(atBottom); if (atBottom) setUnreadCount(0); }} className="min-h-0 flex-1 overflow-y-auto">
                  ...
                  <article key={message.id} className={`group flex gap-3 sm:gap-4 ${isUser ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={isUser ? "max-w-[88%] sm:max-w-[72%]" : "min-w-0 max-w-[calc(100%-2.5rem)] flex-1 sm:max-w-[88%]"}>
                      ...
                      <div className={`mt-1.5 flex items-center gap-1 text-[9px] text-muted-foreground/45 ${isUser ? "justify-end" : "justify-start"}`}>
                        ...
                        {!isUser && message.time_ms ? <span>· {message.time_ms} ms</span> : null}
      ...
      <div className="pointer-events-none fixed bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-background via-background/92 to-transparent pb-4 pt-12 sm:pb-5">
        <div className="pointer-events-auto mx-auto w-full max-w-3xl px-3 sm:px-6">
          <form onSubmit={(event) => { event.preventDefault(); void sendMessage(input); }} className="rounded-[24px] border border-border/70 bg-background/95 p-2 shadow-[0_10px_35px_rgba(0,0,0,0.07)] backdrop-blur-xl dark:shadow-[0_14px_42px_rgba(0,0,0,0.28)]">
