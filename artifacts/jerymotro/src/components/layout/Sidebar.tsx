  ...
  const sidebarContent = (
    <div className="flex h-full min-h-0 flex-col bg-sidebar text-sidebar-foreground">
      <div className="flex h-16 shrink-0 items-center border-b border-sidebar-border px-3 sm:h-[68px]">
        {collapsed ? (
          <button type="button" onClick={toggle} aria-label={t("common.open")} className="mx-auto flex h-11 w-11 items-center justify-center rounded-xl outline-none transition-colors hover:bg-sidebar-accent focus-visible:ring-2 focus-visible:ring-primary">
      ...
      <nav aria-label="Application navigation" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-2 py-3 sm:px-3 sm:py-4" style={{ scrollbarGutter: "stable", paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
        <div className={collapsed ? "space-y-3" : "space-y-4"}>
          {navGroups.filter((group) => !group.authOnly || isAuthenticated).map((group, groupIndex) => (
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={isMobile ? close : undefined} title={collapsed ? t(item.labelKey) : undefined} aria-current={active ? "page" : undefined} className={`group relative flex min-h-11 items-center rounded-xl text-[13px] font-semibold outline-none transition-[background-color,color,transform] duration-150 hover:translate-x-px focus-visible:ring-2 focus-visible:ring-primary ${active ? "bg-sidebar-primary text-sidebar-primary-foreground shadow-sm" : "text-sidebar-foreground/72 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"} ${collapsed ? "mx-auto w-11 justify-center" : "w-full gap-3 px-3"}`}>
                      {active && !collapsed && <span aria-hidden className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-primary-foreground/85" />}
                      <Icon className="h-[18px] w-[18px] shrink-0" />
                      {!collapsed && <><span className="min-w-0 flex-1 truncate text-left">{t(item.labelKey)}</span>{item.locked && <span className="rounded-md bg-primary/10 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-primary">PRO</span>}{item.badge && <span className="inline-flex items-center gap-1 rounded-md bg-primary/12 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-primary"><Sparkles className="h-2.5 w-2.5" />{item.badge}</span>}</>}
                      {collapsed && item.badge && <span aria-hidden className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-primary ring-2 ring-sidebar" />}
                    </Link>
                  );
                })}
              </div>
