...
import { Activity, LayoutDashboard, Flame, BarChart3, Bot, MapPin, Bell, CreditCard, User, LogOut, ChevronLeft, ChevronRight, X, Map, BookOpen, FileText, Shield, Download, UserSquare, Sparkles, Languages, Sun, Moon } from "lucide-react";
...
const navGroups: NavGroup[] = [
  ...
  { sectionKey: "nav.section.detections", authOnly: true, items: [{ labelKey: "nav.detections", href: "/detections", icon: Activity }, { labelKey: "nav.clusters", href: "/clusters", icon: Flame }] },
  { sectionKey: "nav.section.analyse", authOnly: true, items: [{ labelKey: "nav.dashboard", href: "/dashboard", icon: LayoutDashboard }, { labelKey: "nav.predictions", href: "/predictions", icon: MapPin }, { labelKey: "nav.stats", href: "/stats", icon: BarChart3 }, { labelKey: "nav.chat", href: "/chat", icon: Bot, badge: "AI" }] },
  { sectionKey: "nav.section.premium", authOnly: true, items: [{ labelKey: "nav.zones", href: "/zones", icon: MapPin, locked: true }] },
  { sectionKey: "nav.section.account", authOnly: true, items: [{ labelKey: "nav.alerts", href: "/alerts", icon: Bell }, { labelKey: "nav.subscriptions", href: "/subscriptions", icon: CreditCard }, { labelKey: "nav.profile", href: "/profile", icon: User }, { labelKey: "nav.export", href: "/export", icon: Download }] },
  { sectionKey: "nav.section.resources", items: [{ labelKey: "nav.about", href: "/about", icon: BookOpen }, { labelKey: "nav.cv", href: "/cv", icon: UserSquare }, { labelKey: "nav.legal", href: "/legal", icon: FileText }, { labelKey: "nav.privacy", href: "/privacy", icon: Shield }] },
];
...
export function Sidebar() {
  ...
  const sidebarContent = (
    <div className="flex h-full min-h-0 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <nav aria-label="Primary navigation" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-2 py-3 sm:px-3 sm:py-4" style={{ scrollbarGutter: "stable", paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
        <div className={collapsed ? "space-y-3" : "space-y-4"}>
          {navGroups.filter((group) => !group.authOnly || isAuthenticated).map((group, groupIndex) => <section key={group.sectionKey} aria-label={collapsed ? undefined : t(group.sectionKey)}>
            {!collapsed ? <h2 className="mb-1.5 px-3 text-[10px] font-bold uppercase tracking-[0.14em] text-sidebar-foreground/40">{t(group.sectionKey)}</h2> : groupIndex > 0 && <div className="mx-auto mb-2 h-px w-8 bg-sidebar-border/70" />}
            <div className="space-y-0.5">{group.items.map((item) => { const Icon = item.icon; const active = isActive(item.href); return <Link key={item.href} href={item.href} onClick={isMobile ? close : undefined} title={collapsed ? t(item.labelKey) : undefined} aria-current={active ? "page" : undefined} className={`group relative flex min-h-11 items-center rounded-xl text-[13px] font-semibold outline-none transition-[background-color,color,transform] duration-150 hover:translate-x-px focus-visible:ring-2 focus-visible:ring-primary ${active ? "bg-sidebar-primary text-sidebar-primary-foreground shadow-sm" : "text-sidebar-foreground/72 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"} ${collapsed ? "mx-auto w-11 justify-center" : "w-full gap-3 px-3"}`}>
              {active && !collapsed && <span aria-hidden className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-primary-foreground/85" />}<Icon className="h-[18px] w-[18px] shrink-0" />{!collapsed && <><span className="min-w-0 flex-1 truncate text-left">{t(item.labelKey)}</span>{item.locked && <span className="text-[10px] font-bold text-amber-500">PRO</span>}{item.badge && <span className="inline-flex items-center gap-1 rounded-md bg-primary/12 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-primary"><Sparkles className="h-2.5 w-2.5" />{item.badge}</span>}</>}{collapsed && item.badge && <span aria-hidden className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-primary ring-2 ring-sidebar" />}
            </Link>; })}</div>
          </section>)}
        </div>
      </nav>
      ...
      <div className={`shrink-0 border-t border-sidebar-border ${collapsed ? "px-2 py-2" : "p-3"}`} style={{ paddingBottom: collapsed ? "max(0.5rem, env(safe-area-inset-bottom))" : "max(0.75rem, env(safe-area-inset-bottom))" }}>
        {!collapsed && <div className="mb-2 flex items-center gap-1 rounded-xl bg-sidebar-accent/60 p-1">
          ...
          <select value={lang} onChange={(e) => setLang(e.target.value as any)} aria-label="Langue" className="h-9 min-w-0 flex-1 cursor-pointer bg-transparent px-1 text-xs font-semibold text-sidebar-foreground outline-none"><option value="fr">FR</option><option value="mg">MG</option><option value="en">EN</option></select>
