  ...
  return (
    <header className="fixed right-0 top-0 z-10 flex h-[58px] items-center justify-between border-b border-border/60 bg-background/90 px-3 backdrop-blur-xl transition-[left] duration-300 sm:px-5" style={{ left: leftOffset }}>
      <div className="flex min-w-0 items-center gap-2 sm:gap-3">
        <button type="button" onClick={toggle} className="jm-topbar-control flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Toggle sidebar"><Menu className="h-[18px] w-[18px]" /></button>
      ...
      <div className="flex shrink-0 items-center gap-1 sm:gap-1.5">
        ...
        <div className="jm-topbar-segment flex h-8 items-center gap-0.5 rounded-xl border border-transparent bg-muted/40 px-1" aria-label="Language">
          <Globe className="ml-1 mr-0.5 hidden h-3.5 w-3.5 text-muted-foreground/60 sm:block" aria-hidden="true" />
          {LANGS.map((l) => <button key={l} type="button" onClick={() => setLang(l)} title={LANG_LABELS[l]} aria-pressed={lang === l} className={`min-h-7 min-w-7 rounded-md px-1.5 text-[9px] font-bold uppercase transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${lang === l ? "jm-topbar-segment-active bg-background text-foreground" : "text-muted-foreground/55 hover:bg-background/70 hover:text-foreground"}`}>{l}</button>)}
        </div>

        <button type="button" onClick={toggleTheme} className="jm-topbar-control flex h-9 w-9 items-center justify-center rounded-xl border border-transparent bg-muted/40 text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={t("topbar.themeToggle" as any) || "Toggle theme"}>
          {theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
        </button>

        <button type="button" className="jm-topbar-control relative flex h-9 w-9 items-center justify-center rounded-xl border border-transparent bg-muted/40 text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={t("nav.alerts")}><Bell className="h-4 w-4" /><span aria-hidden className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-destructive ring-2 ring-background" /></button>
