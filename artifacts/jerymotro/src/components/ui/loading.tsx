import { useI18n } from "@/hooks/use-i18n";

interface LoadingPageProps {
  message?: string;
}

interface AnimatedTextProps {
  text: string;
  stepMs?: number;
  className?: string;
}

function AnimatedText({
  text,
  stepMs = 42,
  className = "",
}: AnimatedTextProps) {
  return (
    <span aria-label={text} className={className}>
      {Array.from(text).map((letter, index) => (
        <span
          key={`loading-letter-${index}`}
          aria-hidden="true"
          className="jm-loading-letter inline-block will-change-[transform,opacity,filter]"
          style={{
            animationDelay: `${Math.min(index * stepMs, 760)}ms`,
          }}
        >
          {letter === " " ? "\u00A0" : letter}
        </span>
      ))}
    </span>
  );
}

export default function LoadingPage({ message }: LoadingPageProps) {
  const { t } = useI18n();
  const loadingMessage = message ?? t("common.loading");

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-background px-6">
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        <div className="absolute -left-24 -top-24 h-72 w-72 rounded-full bg-primary/10 blur-3xl" />
        <div className="absolute -bottom-28 -right-20 h-80 w-80 rounded-full bg-accent/10 blur-3xl" />
        <div className="absolute left-1/2 top-1/2 h-96 w-96 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/[0.035] blur-3xl" />
      </div>

      <div className="jm-loading-surface relative w-full max-w-sm overflow-hidden border border-border/60 bg-card/90 p-7 shadow-2xl backdrop-blur-xl sm:p-8">
        <div className="jm-loading-piece mb-6 flex items-center gap-3" style={{ animationDelay: "0ms" }}>
          <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-primary/10 ring-1 ring-primary/20">
            <div className="absolute inset-1 rounded-xl bg-gradient-to-br from-primary/15 to-accent/10" />
            <img src="/logo.png" alt="JeryMotro" className="jm-logo-mark relative h-10 w-10 object-contain" />
          </div>
          <div className="min-w-0">
            <h3 className="font-heading text-2xl font-bold leading-none tracking-[-0.035em] text-foreground transition-colors sm:text-3xl">
              <AnimatedText text="JeryMotro" stepMs={68} />
            </h3>
            <div className="mt-1 text-xs text-muted-foreground">
              <AnimatedText text="Surveillance des feux de brousse" stepMs={25} />
            </div>
          </div>
        </div>

        <div className="jm-loading-piece mb-6 h-1.5 overflow-hidden rounded-full bg-secondary" style={{ animationDelay: "140ms" }} aria-hidden="true">
          <div className="h-full w-1/3 rounded-full bg-gradient-to-r from-primary via-primary to-accent animate-[loading_1.6s_ease-in-out_infinite]" />
        </div>

        <div className="jm-loading-piece flex items-center gap-3" style={{ animationDelay: "220ms" }}>
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10" aria-hidden="true">
            <span className="h-4 w-4 animate-spin rounded-full border-2 border-primary/25 border-t-primary" />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-medium text-foreground">
              <AnimatedText text={loadingMessage} stepMs={28} />
            </p>
            <p className="mt-0.5 text-xs leading-5 text-muted-foreground">
              <AnimatedText text="Préparation de votre espace de surveillance…" stepMs={22} />
            </p>
          </div>
        </div>

        <div className="jm-loading-piece mt-6 flex items-center gap-1.5" style={{ animationDelay: "320ms" }}>
          <span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" aria-hidden="true" />
          <span className="h-1.5 w-1.5 rounded-full bg-primary/70 animate-pulse [animation-delay:150ms]" aria-hidden="true" />
          <span className="h-1.5 w-1.5 rounded-full bg-accent animate-pulse [animation-delay:300ms]" aria-hidden="true" />
          <span className="ml-1 text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
            <AnimatedText text="Connexion sécurisée" stepMs={20} />
          </span>
        </div>
      </div>

      <style>{`
        @keyframes loading {
          0%, 100% {
            transform: translateX(-120%);
            opacity: .45;
          }
          50% {
            transform: translateX(250%);
            opacity: 1;
          }
        }

        @keyframes jm-loading-letter-reveal {
          0%, 100% {
            opacity: 0;
            transform: translate3d(0, 0.65em, 0) scale(0.96);
            filter: blur(10px);
          }
          18%, 76% {
            opacity: 1;
            transform: translate3d(0, 0, 0) scale(1);
            filter: blur(0);
          }
        }

        @keyframes jm-loading-piece-reveal {
          from {
            opacity: 0;
            transform: translate3d(0, 18px, 0) scale(0.985);
            filter: blur(8px);
          }
          to {
            opacity: 1;
            transform: translate3d(0, 0, 0) scale(1);
            filter: blur(0);
          }
        }

        .jm-loading-letter {
          animation: jm-loading-letter-reveal 2.8s cubic-bezier(0.22, 1, 0.36, 1) infinite both;
        }

        .jm-loading-piece {
          animation: jm-loading-piece-reveal 700ms cubic-bezier(0.22, 1, 0.36, 1) both;
        }

        @media (prefers-reduced-motion: reduce) {
          .jm-loading-letter,
          .jm-loading-piece {
            animation: none;
            opacity: 1;
            transform: none;
            filter: none;
          }
        }
      `}</style>
    </div>
  );
}
